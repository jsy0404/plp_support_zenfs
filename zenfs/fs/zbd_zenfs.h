...
class Zone {
 ...
 public:
  ...
  __u32 Pwrite2Ioctl(int, void *, uint32_t, uint64_t, bool);
  bool IsUsed();
  bool IsFull();
  bool IsEmpty();
  uint64_t GetZoneNr();
...
class ZonedBlockDevice {
 private:
  ...
  uint32_t zone_sz_;
  uint32_t nr_zones_;
  uint32_t nr_plp_zones_;
  uint32_t zasl_;
  std::vector<Zone *> io_zones;
  std::vector<Zone *> plp_zones;
 ...
 public:
  ...
  Zone *AllocateZone(Env::WriteLifeTimeHint lifetime, bool plp_flag);
